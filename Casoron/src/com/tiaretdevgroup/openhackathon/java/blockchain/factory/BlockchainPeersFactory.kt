...
object BlockchainPeersFactory {
    ...
    fun addBlockToPeer(ipAddress: String, block: Block) {
        println(block.toStringBlockChain())
        Unirest.post("http://$ipAddress:5000/${if (block is MaladyBlock) "addMaladyBlock" else "addSaleBlock"}")
                .queryString("block",block.toStringBlockChain())
                .asString()
    }
}